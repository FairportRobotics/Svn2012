...
public class Camera {
    ...
    public Camera() {
        ...
        SmartDashboard.putBoolean("Has Image: ", hasImage);
    }
...
//            eqRecHeight = (int) NIVision.MeasureParticle(image.image,  NIVision.countParticles(image.image), false, MeasurementType.IMAQ_MT_EQUIVALENT_RECT_SHORT_SIDE);
    ...
    public double getMotorDirection(int target) {
        try {
            if (targets[target].center_mass_x > 160) {
                return -0.5;
            } else if (targets[target].center_mass_x < 160) {
                return 0.5;
            }
        } 
        catch (NullPointerException e)
