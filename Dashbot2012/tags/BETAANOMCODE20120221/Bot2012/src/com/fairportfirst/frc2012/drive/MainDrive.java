...
//    private DigitalInput turretCcwLimit;
    ...
    AutonomousChooser autoType;
    SendableChooser autonomousChooser;

    private static int STATIONARY_SHOOT = 1;
    private static int TIP_THEN_SHOOT = 2;
    private static int SHOOT_THEN_TIP = 3;
    private static int JUST_BRIDGE = 4;
    private static int DO_NOTHING = 5;

    AutonomousHoop autoHoop;
    SendableChooser shootAtHoop;

    private static int TOP_HOOP = 1;
    private static int MIDDLE_LEFT_HOOP = 2;
    private static int MIDDLE_RIGHT_HOOP = 3;
    private static int BOTTOM_HOOP = 4;

    AutonomousPlace autoPlace;
    SendableChooser placeAtAuto;
    ...
    int anomOrder[] = new int[4];
    double anomDelay[] = new double[4];
    int anomTask = 0;
    double startTime = 0.0;
    boolean taskOneC = false;
    boolean taskTwoC = false;
...
//        double angle = roundTwoPlaces(mrGyro.getAngle());
        ...
        autonomousChooser.addDefault("Default Stationary Shoot", new AutonomousChooser(STATIONARY_SHOOT));
        autonomousChooser.addObject("Tip bridge, then shoot", new AutonomousChooser(TIP_THEN_SHOOT));
        autonomousChooser.addObject("Shoot, then tip bridge", new AutonomousChooser(SHOOT_THEN_TIP));
        autonomousChooser.addObject("Just tip bridge", new AutonomousChooser(JUST_BRIDGE));
        autonomousChooser.addObject("Do nothing", new AutonomousChooser(DO_NOTHING));
        SmartDashboard.putData("Autonomous Chooser", autonomousChooser);

        shootAtHoop = new SendableChooser();
        shootAtHoop.addDefault("Top Hoop", new AutonomousHoop(TOP_HOOP));
        shootAtHoop.addObject("Middle-Left", new AutonomousHoop(MIDDLE_LEFT_HOOP));
        shootAtHoop.addObject("Middle-Right", new AutonomousHoop(MIDDLE_RIGHT_HOOP));
        shootAtHoop.addObject("Bottom", new AutonomousHoop(BOTTOM_HOOP));
        SmartDashboard.putData("Autonomous Hoop", shootAtHoop);

        placeAtAuto = new SendableChooser();
        placeAtAuto.addDefault("Left", new AutonomousPlace(LEFT));
        placeAtAuto.addObject("Middle", new AutonomousPlace(MIDDLE));
        placeAtAuto.addObject("Right", new AutonomousPlace(RIGHT));
        SmartDashboard.putData("Autonomous Place", placeAtAuto);
    }
    ...
    public void autonomousInit()
    {
        ...
        autoType = (AutonomousChooser) autonomousChooser.getSelected();
        autoHoop = (AutonomousHoop) shootAtHoop.getSelected();
        autoPlace = (AutonomousPlace) placeAtAuto.getSelected();
...
//        Timer.delay(3.0);
    ...
    public void anomSenario(int q)
    {
        if(anomOrder[q] == 0)
        {
         //   Timer.delay(anomDelay[q]);
        }
        else if(anomOrder[q] == 1)
        {
            anomShoot();
        }
        else if(anomOrder[q] == 2)
        {
            anomDrive();
        }
        if(hasTimePassed(anomDelay[q]) && anomTask < 5)
        {
            anomTask ++;
        }
    }
    public void anomShoot()
    {
        mrShooter.setWheelSpeed(0.66);
        mrLift.up();
    }
    public void anomDrive()
    {
        //TODO: PUT DRIVE HERE
    }
...
//        dslcd.println(DriverStationLCD.Line.kUser5, 1,"");
    ...
    public class AutonomousChooser {
        public int autoChoose;
        public AutonomousChooser(int type) {
            autoChoose = type;
        }
        public int getType() {
            return autoChoose;
    ...
    public class AutonomousHoop {
        public int whichHoop;
        public AutonomousHoop(int hoop) {
            whichHoop = hoop;
        }
        public int getHoop() {
            return whichHoop;
    ...
    public class AutonomousPlace {
        public int whichPlace;
        public AutonomousPlace(int place) {
            whichPlace = place;
        }
        public int getPlace() {
            return whichPlace;
