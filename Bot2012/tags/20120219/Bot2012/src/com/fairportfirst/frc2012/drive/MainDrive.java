...
//        }
    ...
    public void moveRobot() {
        if (driveType.getDriveType() == TANK_DRIVE) {
            left = roundDirectionNum(driveStick1.getY());
            right = roundDirectionNum(-driveStick2.getY());
//            if((driveStick1.getX() < 0 && driveStick2.getX() > 0) || (driveStick1.getX() > 0 && driveStick2.getX() < 0)) {
//                left = 0;
//                right = 0;
//            }
        } else if (driveType.getDriveType() == TILT_DRIVE) {
            left = roundDirectionNum(driveStick1.getY() - driveStick1.getX());
...
//        right*= ((driveStick1.getThrottle()+1)/2);
        left = roundDirectionNum(left);
        right = roundDirectionNum(right);
...
//        }
    ...
    public void aimTurret() {
        ...
        else
        {
            ...
            double moverMass = ((massX -160.0)/160.0) + 0.05;
...
//        }
    ...
    public void camera()
    {
        ...
        if(camera.freshImage())
        {
            try
            {
                 massX = spyCam.calImage(camera.getImage(), 0);
            }
        ...
        SmartDashboard.putInt("Distance", (int)spyCam.getDistance(0));
