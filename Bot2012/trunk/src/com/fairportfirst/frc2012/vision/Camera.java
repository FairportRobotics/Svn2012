...
public class Camera {
    ...
    //PROTIP: use maxDistance in inches
    ...
    public double getDistance(int target) {
        try
        {
            ...
            //double pthag = ((heigth * heigth) + (targets[target].boundingRectWidth * targets[target].boundingRectWidth));
    ...
    public int getMotorDirection(int target) {
        try {
            if (targets[target].center_mass_x > 158) {
                return -1;
            } else if (targets[target].center_mass_x < 162) {
                return 1;
            }
        } 
        catch (NullPointerException e)
    ...
    public double distanceToMotor(double darstance)
    {
        //returns a motor value for the shooter wheels to use based on
        //distance
        if(darstance < maxDistance)
        {
            //checks to make sure the given distance is within range
            System.out.println("Im sorry sir but you must be this short to ride the shooter");
            return 0.0;
        }
        return darstance/maxDistance;
    }
}
